...
function draw() {
  ...
  if (gameState===PLAY){
    ...
    if(keyDown("space") && trex.y >= (height/2)-20) {
      trex.velocityY = -12;
    }
...
function spawnClouds() {
  ...
  if (frameCount % 60 === 0) {
    var cloud = createSprite(width -10,height/4,40,10);
    cloud.y = Math.round(random(height/4 -30,height/4 +30));
    cloud.addImage(cloudImage);
    cloud.scale = 0.5;
    cloud.velocityX = -3;
    
     //assign lifetime to the variable
    cloud.lifetime = width/Math.abs(cloud.velocityX);
...
function spawnObstacles() {
  if(frameCount % 60 === 0) {
    var obstacle = createSprite(width -10,(height/2)-15,10,40);
    //obstacle.debug = true;
    ...
    obstacle.scale = 0.5;
    obstacle.lifetime = width/Math.abs(obstacle.velocityX);
